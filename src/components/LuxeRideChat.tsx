  ...
  const sendMessage = async (msg: string) => {
    ...
    try {
      const res = await fetch("/functions/v1/chatbot", {
        ...
        body: JSON.stringify({ message: msg, user_id: "demo-user" }),
      });
      const data = await res.json();
      setMessages((prev) => [
        ...prev,
        { role: "bot", content: data.response || "Sorry, I didn't understand. Please rephrase or ask about LuxeRide services." },
      ]);
      setShowQuickReplies(true);
    } catch (e) {
      setMessages((prev) => [
        ...prev,
        { role: "bot", content: "Sorry, our assistant is temporarily unavailable. Please try again later." },
      ]);
    } finally {
      setLoading(false);
    }
