  ...
  try {
    ...
    const content = `
      ...
      <div class="info-box" style="background: linear-gradient(135deg, #FFF9E6 0%, #FFF5D6 100%); border-left: 4px solid #D4AF37;">
        ...
        ${paymentMethod ? `<p style="margin: 10px 0;"><strong style="color: #D4AF37;">💳 Payment Method:</strong> ${paymentMethod}</p>` : ""}
        ${transactionId ? `<p style="margin: 10px 0;"><strong style="color: #D4AF37;">🔢 Transaction ID:</strong> ${transactionId}</p>` : ""}
      </div>
      ...
      <div class="message" style="background: linear-gradient(135deg, #F0F9FF 0%, #E0F2FE 100%); padding: 25px; border-radius: 8px; border-left: 4px solid #3B82F6;">
        <strong style="color: #3B82F6; font-size: 20px;">🎁 Your ${packageName} Package Benefits:</strong>
        <ul style="margin-top: 15px; padding-left: 25px; line-height: 2;">
          ${benefits.map(benefit => `<li style="margin: 10px 0;">${benefit}</li>`).join('')}
        </ul>
      </div>
      ...
      <div class="note" style="background-color: #F9FAFB;">
        ...
        <a href="mailto:support@luxeride.org" style="color: #D4AF37; font-weight: 600;">support@luxeride.org</a>
      </div>
    `;
