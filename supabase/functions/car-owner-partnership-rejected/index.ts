// Edge Function: Send Car Owner Partnership Rejected Email
// Triggered when admin rejects car owner partnership
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const content = `
      ...
      <div class="message">
        Thank you ${ownerName || "there"} for your interest in partnering with LuxeRide. After careful review, we're unable to approve your partnership application at this time.
      </div>
      ...
      <div class="note" style="background-color: #F9FAFB;">
        <strong>💬 Questions?</strong> If you have questions about this decision, contact our partnership team at 
        <a href="mailto:${supportContact || 'partners@luxeride.org'}" style="color: #EF4444; font-weight: 600;">${supportContact || 'partners@luxeride.org'}</a>
      </div>
    `;
    ...
    const result = await sendEmail({
      to: ownerEmail,
      subject: "Partnership Application Status Update",
      html,
    });
    ...
    console.error("Error sending car owner partnership rejected email:", error);
